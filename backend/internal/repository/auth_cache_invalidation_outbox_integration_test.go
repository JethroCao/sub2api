...
func TestAuthCacheInvalidationTriggers_CoverSecurityMutationsOnly(t *testing.T) {
	...
	clear()
	_, err = integrationDB.ExecContext(ctx, "UPDATE groups SET allow_video_generation = NOT allow_video_generation WHERE id = $1", group.ID)
	require.NoError(t, err)
	require.Equal(t, 1, count(), "video-generation permission changes must enqueue bound keys")
	clear()
