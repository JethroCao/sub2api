...
// ValidateResolvedIPs applies the outbound destination policy to already
// resolved addresses. Keeping the check separate makes resolver/dial boundary
// callers deterministic and prevents public hostnames from rebinding to
...
func ValidateResolvedIPs(ips []net.IP) error {
	...
	for _, ip := range ips {
		...
		if !ok || !address.IsValid() {
			return errors.New("resolved address is invalid")
		}
		address = address.Unmap()
		for _, prefix := range blockedResolvedIPPrefixes {
			if prefix.Contains(address) {
				return fmt.Errorf("resolved ip %s is not allowed", address.String())
			}
		}
	}
	return nil
