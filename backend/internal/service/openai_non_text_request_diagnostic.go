...
func isOpenAINonTextInputUnsupportedError(upstreamStatusCode int, upstreamMsg string, upstreamBody []byte) bool {
	...
	return strings.Contains(message, "model only support text input") ||
		strings.Contains(message, "model do not support image input")
}

func logOpenAINonTextInputRequestBodies(
	ctx context.Context,
	c *gin.Context,
	account *Account,
	...
	if !isOpenAINonTextInputUnsupportedError(upstreamStatusCode, upstreamMsg, upstreamErrorBody) {
		return
	}
	if ctx == nil {
	...
	logBody := func(bodyKind string, body []byte) {
		...
		logger.FromContext(ctx).With(fields...).Warn(openAINonTextInputRequestBodyLogMessage)
	}
