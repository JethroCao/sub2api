...
func (s *accountRepoStubForGroupCodexManifest) GetByIDs(_ context.Context, ids []int64) ([]*Account, error) {
	byID := make(map[int64]Account, len(s.accounts))
...
func TestAdminService_UpdateGroup_CopyAccountsRejectsStaleCodexManifestPins(t *testing.T) {
	existing := &Group{
		ID:       1,
		Name:     "target",
		Platform: PlatformOpenAI,
	...
	source := &Group{ID: 2, Name: "source", Platform: PlatformOpenAI, Status: StatusActive}
	repo := &groupRepoStubForAdmin{
		getByIDByID: map[int64]*Group{1: existing, 2: source},
		getAccountIDsByGroupIDsFn: func(groupIDs []int64) ([]int64, error) {
			require.Equal(t, []int64{2}, groupIDs)
			return []int64{20}, nil
		},
		deleteAccountGroupsByGroupIDFn: func(int64) (int64, error) {
			t.Fatal("account bindings must not change when manifest pins are invalid")
			return 0, nil
		},
	...
	svc := &adminServiceImpl{
		groupRepo: repo,
		accountRepo: &accountRepoStubForGroupCodexManifest{
			accounts: openAIGroupCodexManifestAccounts(10, 20),
		},
	}
	...
	require.Equal(t, http.StatusBadRequest, infraerrors.Code(err))
	require.Equal(t, "INVALID_CODEX_MODELS_MANIFEST_CONFIG", infraerrors.Reason(err))
	require.Nil(t, repo.updated, "group config must not persist before post-copy membership validation")
}
