...
func TestOpenAIWSIngressAccountCustomInstructionsEveryCreate(t *testing.T) {
	gin.SetMode(gin.TestMode)

	const suffix = "account websocket suffix"
	const firstClientInstructions = "  first client instructions  "
	const followupClientInstructions = "\tsecond client instructions\n"
	...
	captureConn := &openAIWSCaptureConn{events: [][]byte{
		[]byte(`{"type":"response.completed","response":{"id":"resp_account_instructions_1","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		[]byte(`{"type":"response.completed","response":{"id":"resp_account_instructions_2","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
		[]byte(`{"type":"response.completed","response":{"id":"resp_account_instructions_3","model":"gpt-5.1","usage":{"input_tokens":1,"output_tokens":1}}}`),
	}}
	...
	account := &Account{
		ID:          117,
		Name:        "openai-ingress-account-instructions",
		...
		Credentials: map[string]any{
			"api_key":                             "sk-test",
			OpenAICustomInstructionsCredentialKey: suffix,
		},
	...
	writeAndRead := func(payload string, wantResponseID string) {
		t.Helper()
	...
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","stream":false,"instructions":"  first client instructions  "}`, "resp_account_instructions_1")
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_account_instructions_1","instructions":"\tsecond client instructions\n"}`, "resp_account_instructions_2")
	writeAndRead(`{"type":"response.create","model":"gpt-5.1","stream":false,"previous_response_id":"resp_account_instructions_2","instructions":"\tsecond client instructions\n\n\naccount websocket suffix"}`, "resp_account_instructions_3")
	...
	require.Equal(t, firstClientInstructions+"\n\n"+suffix, gjson.Get(requestToJSONString(captureConn.writes[0]), "instructions").String())
	require.Equal(t, followupClientInstructions+"\n\n"+suffix, gjson.Get(requestToJSONString(captureConn.writes[1]), "instructions").String())
	require.Equal(t, followupClientInstructions+"\n\n"+suffix, gjson.Get(requestToJSONString(captureConn.writes[2]), "instructions").String())
	for _, write := range captureConn.writes {
		require.Equal(t, 1, strings.Count(gjson.Get(requestToJSONString(write), "instructions").String(), suffix))
	}
}
