...
func (s *adminServiceImpl) validateCodexModelsManifestConfig(ctx context.Context, groupID int64, cfg GroupCodexModelsManifestConfig) error {
	...
	return validateCodexModelsManifestConfigAgainstAccounts(cfg, accountPtrs)
}

func (s *adminServiceImpl) validateCodexModelsManifestConfigForAccountIDs(ctx context.Context, accountIDs []int64, cfg GroupCodexModelsManifestConfig) error {
	if !cfg.Enabled {
		return nil
	}
	accounts, err := s.accountRepo.GetByIDs(ctx, accountIDs)
	if err != nil {
		return fmt.Errorf("load copied accounts for codex models manifest config: %w", err)
	}
	return validateCodexModelsManifestConfigAgainstAccounts(cfg, accounts)
}

func validateCodexModelsManifestConfigAgainstAccounts(cfg GroupCodexModelsManifestConfig, accounts []*Account) error {
	if len(cfg.AccountIDs) == 0 {
