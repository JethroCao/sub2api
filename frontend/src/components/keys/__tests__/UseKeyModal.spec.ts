...
describe('UseKeyModal', () => {
  ...
  it('uses the Video provider label in routed Codex configuration', async () => {
    const wrapper = mount(UseKeyModal, {
      props: {
        show: true,
    ...
    const config = wrapper.findAll('pre code')
      .map((code) => code.text())
      .find((content) => content.includes('[model_providers.sub2api]'))
    expect(config).toContain('name = "Sub2API Video"')
    expect(config).not.toContain('Sub2API undefined')
  })
