<template>
  ...
  >
    <form id="bulk-edit-account-form" class="space-y-5" @submit.prevent="() => handleSubmit()">
      ...
      <!-- OpenAI API Key Responses compatibility flags -->
      <div v-if="allOpenAIAPIKey" class="space-y-4 border-t border-gray-200 pt-4 dark:border-dark-600">
        <div class="flex items-center justify-between gap-4">
          <div class="flex-1">
            <label class="input-label mb-0" for="bulk-edit-openai-responses-message-partial-compat-enabled">
              {{ t('admin.accounts.openai.responsesMessagePartialCompat') }}
            </label>
            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">
          ...
          <input
            v-model="enableOpenAIResponsesMessagePartialCompat"
            id="bulk-edit-openai-responses-message-partial-compat-enabled"
            type="checkbox"
            class="rounded border-gray-300 text-primary-600 focus:ring-primary-500"
          />
          <button
            type="button"
            :disabled="!enableOpenAIResponsesMessagePartialCompat"
            data-testid="bulk-edit-openai-responses-message-partial-compat-toggle"
            :aria-pressed="openAIResponsesMessagePartialCompatEnabled"
            :class="[
              'relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2',
              openAIResponsesMessagePartialCompatEnabled
                ? 'bg-primary-600'
                : 'bg-gray-200 dark:bg-dark-600'
            ]"
            @click="openAIResponsesMessagePartialCompatEnabled = !openAIResponsesMessagePartialCompatEnabled"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
                openAIResponsesMessagePartialCompatEnabled ? 'translate-x-5' : 'translate-x-0'
              ]"
            />
          </button>
        </div>
        <div class="flex items-center justify-between gap-4">
          <div class="flex-1">
            <label class="input-label mb-0" for="bulk-edit-openai-responses-assistant-prefill-compat-enabled">
          ...
          <button
            type="button"
            :disabled="!enableOpenAIResponsesAssistantPrefillCompat"
            ...
            @click="openAIResponsesAssistantPrefillCompatEnabled = !openAIResponsesAssistantPrefillCompatEnabled"
          >
            <span
              :class="[
                'pointer-events-none inline-block h-5 w-5 transform rounded-full bg-white shadow ring-0 transition duration-200 ease-in-out',
...
const buildUpdatePayload = (): Record<string, unknown> | null => {
  ...
  if (enableOpenAIResponsesMessagePartialCompat.value) {
    const extra = ensureExtra()
    extra.openai_responses_message_partial_compat_enabled =
