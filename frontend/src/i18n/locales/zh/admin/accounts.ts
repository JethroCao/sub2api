export default {
    accounts: {
      ...
      video: {
        provider: '视频供应商',
        providers: { seedance: 'Seedance', kling: '可灵' },
        apiKey: 'API Key',
        accessKey: 'Access Key',
        secretKey: 'Secret Key',
        apiKeyPlaceholder: '请输入 Seedance API Key',
        accessKeyPlaceholder: '请输入可灵 Access Key',
        secretKeyPlaceholder: '请输入可灵 Secret Key',
        secretConfigured: '密钥已配置',
        baseUrlPlaceholder: 'https://api.example.com',
        baseUrlHint: '可选的 HTTPS 根地址，最终由服务端按供应商规则校验。',
        modelMappingHint: '将每个公开模型名精确映射到一个上游模型或端点 ID，不支持通配符。',
        capabilities: '可用能力',
        capabilitiesUnavailable: 'API 未报告此账号的任何能力。',
        disableCapability: '禁用{capability}',
        capability: {
          audio: '音频', edit: '编辑', extension: '扩展',
          first_and_last_frame: '首尾帧', first_frame: '首帧',
          generation: '生成', last_frame: '尾帧',
          reference_images: '参考图', reference_videos: '参考视频', text: '文本'
        },
        invalidBaseUrl: '请输入有效的供应商 HTTPS 根地址。',
        apiKeyRequired: 'Seedance API Key 为必填项。',
        klingCredentialsRequired: '可灵 Access Key 和 Secret Key 均为必填项。',
        invalidConfiguration: '请完成有效的视频供应商配置，并至少添加一条精确模型映射。',
        invalidMetadata: '该视频账号包含未知或旧版元数据，无法安全编辑。'
      },
      types: {
        oauth: 'OAuth',
      ...
      openai: {
        ...
        responsesMessagePartialCompatDesc:
          '仅对 OpenAI API Key 生效。按火山方舟规则规范化 Responses message：清理无实际内容的空文本历史消息；仅当 input 最后一项是 assistant message 时携带 partial=true，同时兼容省略 type 的简写消息；其余 message 不携带 partial。',
        responsesAssistantPrefillCompat: 'Responses assistant prefill 兼容',
        responsesAssistantPrefillCompatDesc:
          '仅对 OpenAI API Key 生效。用于不支持 assistant prefill 的上游：清理无实际内容的空文本历史消息；若 input 最后一项是 assistant 消息，则保留原消息并追加一条最小 user 继续指令，避免上游返回 400。',
