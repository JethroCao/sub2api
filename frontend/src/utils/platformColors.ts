...
export type Platform = 'anthropic' | 'openai' | 'antigravity' | 'gemini' | 'grok' | 'video' | 'composite'
...
const BORDER_STRONG: Record<Platform, string> = {
  ...
  video: 'border-rose-500/35 dark:border-rose-500/30',
...
const BUTTON: Record<Platform, string> = {
  ...
  video: 'bg-rose-600 text-white hover:bg-rose-700 active:bg-rose-800 dark:bg-rose-600/80 dark:hover:bg-rose-600',
...
function isPlatform(p: string): p is Platform {
  return p === 'anthropic' || p === 'openai' || p === 'antigravity' || p === 'gemini' || p === 'grok' || p === 'video' || p === 'composite'
}
